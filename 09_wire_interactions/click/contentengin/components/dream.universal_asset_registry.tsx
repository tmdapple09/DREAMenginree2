...
export default function UniversalAssetRegistry({
  ...
  const fetchRegistry = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const enriched: EnrichedEntry[] = raw.map((entry) => ({
        ...entry,
        gameAsset:
          entry.object_type === "game_asset"
            ? gameAssetsMap[entry.internal_id]
            : undefined,
        bindings:
          entry.object_type === "game_asset"
            ? bindingsMap[entry.internal_id]
            : undefined,
      }));
      ...
      setError(
        err instanceof Error ? toErrorMessage(err) : "Failed to load registry",
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const stats = useMemo(() => {
    ...
    const gameAssets = entries.filter(
      (e) => e.object_type === "game_asset",
    ).length;
    const withBindings = entries.filter(
      (e) => e.bindings && e.bindings.length > 0,
    ).length;
  ...
  const renderStatCard = (
    ...
    >
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div
          style={{ fontSize: 11, color: "var(--de-text-dim)", fontWeight: 500 }}
        >
          {label}
        </div>
      </div>
            ...
            {isEditing ? (
              ...
              <>
                ...
                >
                  ...
                  {hasBindings && (
                    <span
                      style={{ fontSize: 10, color: "#8b5cf6" }}
                      title="Has control bindings"
                    >
              ...
              <div style={{ marginTop: 14 }}>
                ...
                {entry.gameAsset.config_dna != null && (
                  <div style={{ marginTop: 10 }}>
                    ...
                    >
                      {String(
                        JSON.stringify(entry.gameAsset.config_dna, null, 2),
                      )}
                    </pre>
                  </div>
                )}
    ...
    >
      ...
      >
        ...
        >
          <Search
            className="w-4 h-4"
            style={{ color: "var(--de-text-dim)", flexShrink: 0 }}
          />
          <input
            type="text"
          ...
          <button
            ...
            style={{
              ...
              background:
                viewMode === "grid"
                  ? `${accentColor}15`
                  : "var(--de-surface, rgba(255,255,255,0.85))",
        ...
        >
          ...
          {categories.map((cat) => (
            <button
              key={cat.type}
              onClick={() =>
                setActiveCategory(activeCategory === cat.type ? null : cat.type)
              }
              style={{
                padding: "5px 12px",
                borderRadius: 8,
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                fontWeight: 700,
                opacity:
                  !newType.trim() || !newInternalId.trim() || !newLabel.trim()
                    ? 0.5
                    : 1,
                display: "flex",
                alignItems: "center",
      ...
      {!loading && filteredEntries.length === 0 && (
        ...
        >
          <DatabaseIcon
            className="w-10 h-10"
            style={{ color: "var(--de-text-dim)", opacity: 0.3 }}
          />
          <div
            style={{
              fontSize: 15,
