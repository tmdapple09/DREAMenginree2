...
export interface ValidationReport { gameReady:boolean; profile:ExportProfile; errors:string[]; warnings:string[]; metrics:{ triangles:number; vertices:number; materials:number; textures:number; textureMaxResolution:number; bones:number; maxWeightsPerVertex:number; glbSizeBytes:number } }
...
export interface ContentAsset { id:string; category:ContentAssetCategory; subcategory:string; seed:number; contentenginVersion:string; recipe:ContentRecipe; parts:PartNode[]; materials:MaterialDef[]; shaders:ShaderDef[]; skeleton?:SkeletonDef; animations:AnimationClipDef[]; collision:CollisionBlock; physics?:PhysicsDef; lods:LodDef[]; exportProfile:ExportProfile; validation:ValidationReport }
export type ContentAssetObject = DomainObject<'contentengin.asset', ContentAsset>;
export const CONTENTENGIN_VERSION = '2026.06.13';
