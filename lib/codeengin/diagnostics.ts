...
export async function diagnoseFile(filePath: string): Promise<CodeEnginDiagnostic[]> {
  ...
  parsed.errors.forEach((error, index) => {
    diagnostics.push({
      id: `${file.path}:parser-error:${error.line}:${error.col}:${index}`,
      path: file.path,
      line: error.line,
      col: error.col,
      severity: 'error',
  ...
  parsed.warnings.forEach((warning, index) => {
    diagnostics.push({
      id: `${file.path}:parser-warning:${warning.line}:${warning.col}:${index}`,
      path: file.path,
      line: warning.line,
      col: warning.col,
      severity: 'warning',
  ...
  file.content.split('\n').forEach((line, index) => {
    if (/\bany\b/.test(line) && language === 'typescript') {
      diagnostics.push({
        ...
        severity: 'warning',
        message: 'Explicit any found. Keep it only at deliberate untyped boundaries.',
        source: 'codeengin',
      });
    }
    if (/console\.log\s*\(/.test(line)) {
      diagnostics.push({
        ...
        severity: 'info',
        message: 'Console logging is useful while building, but should not leak into polished user flows.',
        source: 'codeengin',
      });
    }
  });
...
export async function diagnoseWorkspace(startPath = ''): Promise<CodeEnginDiagnostic[]> {
  const files = await listEditableFiles(startPath);
  const diagnostics: CodeEnginDiagnostic[] = [];
  for (const filePath of files) {
    try {
      diagnostics.push(...await diagnoseFile(filePath));
    } catch (error: unknown) {
      diagnostics.push({
        id: `${filePath}:diagnostic-read-error`,
        path: filePath,
        line: 1,
        col: 1,
        severity: 'warning',
