...
describe('fusion cartridge depth upgrades', () => {
  ...
  it('VoidlineGP integrates real velocity/accel/drag, beat-window arc, chain meter, particles', () => {
    ...
    expect(src).toMatch(/starsRef/);                 
    expect(src).toMatch(/Math\.exp\(-2\.4 \* dt\)/); 
    expect(src).toMatch(/restitution|\* 0\.35/);     
    expect(src).toMatch(/motionTrail/);              
  ...
  it('SerpentSiege adds shielded + scout enemies, a wave-5 boss, and ballistic projectiles', () => {
    ...
    expect(src).toMatch(/headAngVelRef/);            
    expect(src).toMatch(/Separation/i);              
    expect(src).toMatch(/p\.kind === 'mortar'/);     
