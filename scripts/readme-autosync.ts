...
export const SECTION_REGISTRY: SectionDescriptor[] = [
  ...
  { id: 'backend-system-core-coresurfaces', title: 'Backend, System, Core & CoreSurfaces', globs: ['backend/**', 'core/**', 'coresurfaces/**', 'system/**', 'app/api/**', 'supabase/**', 'utils/supabase/**'] },
...
function inferFullRebuildDynamicSections(allFiles: string[], registry: SectionDescriptor[]): SectionDescriptor[] {
  ...
  for (const top of topLevelDirs) {
    const title = top.split(/[-_]/g).filter(Boolean).map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(' ');
...
export function runReadmeAutosync(options: { changedFiles: string[]; summaryFile?: string; fullRebuild?: boolean }): AutosyncSummary {
  ...
  const affected = fullRebuild
    ? new Map(sections.map((section) => [section.id, { section, subsections: new Set(Object.keys(section.subsections ?? {})) }]))
  ...
  if (fullRebuild) readme = stripManagedGeneratedSections(readme, sections);
  ...
  if (!fullRebuild) readme = dedupeManagedGeneratedSections(readme, sections);
...
if (process.argv[1] && resolve(process.argv[1]) === __filename) {
  ...
  if (fullRebuild) console.log('readme-autosync: running full README rebuild from live repo state.');
  const summary = runReadmeAutosync({ changedFiles, fullRebuild, summaryFile: summaryFile ? resolve(summaryFile) : undefined });
