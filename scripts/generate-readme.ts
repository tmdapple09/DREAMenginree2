...
function repoContext(): RepoContext {
  ...
  const branch = process.env.GITHUB_HEAD_REF
    ...
    || DEFAULT_BRANCH;
  const commit = (process.env.GITHUB_SHA || git(['rev-parse', 'HEAD']) || 'unknown').slice(0, 40);
  ...
  if (sectionsFile && existsSync(resolve(sectionsFile))) {
    try {
      ...
      if (
        ...
        return { markdown: parsed.markdown, stats: parsed.sections };
      }
    } catch (error) {
...
function recentCommits(context: RepoContext): string[] {
  const history = git(['log', '-8', '--pretty=format:%H%x09%cI%x09%an%x09%s']);
...
function main(): void {
  ...
  const product = productOutput(arg(args, 'sections-file'), files, lineBudget);
  const linkedProduct = linkifyProductMarkdown(product.markdown, files, context);
  ...
  const summary = {
    ...
    contentSha256: createHash('sha256').update(next).digest('hex'),
  };
