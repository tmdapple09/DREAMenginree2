...
import GameReadyScanPanel from '@/components/contentengin/GameReadyScanPanel';
import { exportOBJ } from '@/engins/isosurfaceAssetPipeline';
import { RenderStage, createInlineRenderIntent } from '@/engins/renderengin';
...
export default function ImplicitAssetWorkspace({ onBack }: { onBack?: () => void }) {
  const [surfaceMode, setSurfaceMode] = useState<'edit' | 'render'>('edit');
  ...
  const gameReadyScan = data.mesh?.gameReadyScan ?? null;
  ...
  const renderIntent = useMemo(() => {
    if (!mesh || !strictValid) return null;
    return createInlineRenderIntent('ContentEngin', 'render.asset.preview', {
      assetId: ws.workspace.id,
      ownerId: ws.workspace.ownerId,
      runtimeId: ws.workspace.runtimeId,
      visibility: ws.workspace.visibility,
      fileName: ws.workspace.id + '.obj',
      objSource: exportOBJ(mesh),
  ...
  }, [data.mesh?.diagnostics, gameReadyScan?.certificate, gameReadyScan?.similaritySignature, mesh, strictValid, ws.workspace.id, ws.workspace.ownerId, ws.workspace.runtimeId, ws.workspace.visibility]);
  ...
  `}</style>
    <section className="toolbar">
      ...
      <div className="actions">
        ...
        <button disabled={!canEdit || data.processingStatus === 'processing'} onClick={ws.makeGameReady}>{data.processingStatus === 'processing' ? 'Scanning…' : 'Make Game Ready'}</button>
        ...
        <button disabled={!canDownloadObj} onClick={() => ws.download('obj')}>Download OBJ</button>
        <button disabled={!canPreviewInRender} onClick={() => setSurfaceMode('render')}>Render</button>
        <div className="surface-toggle"><button data-active={surfaceMode === 'edit'} onClick={() => setSurfaceMode('edit')}>Edit</button><button data-active={surfaceMode === 'render'} disabled={!canPreviewInRender} onClick={() => setSurfaceMode('render')}>Render</button></div>
        <button disabled={!data.sourceImage && !mesh} onClick={ws.clearWorkspace}>Remove</button>
      </div>
    </section>
    <section className="stage">
      <div className="viewport-shell">{surfaceMode === 'render' && renderIntent ? <RenderStage intent={renderIntent} runtimeId={`render:content:${ws.workspace.id}`} persistenceKey={`render:content:${ws.workspace.id}`} /> : <AssetViewport mesh={mesh} sourceUrl={data.sourceImage?.url} camera={data.cameraState} editMode={data.processingStatus === 'editing'} pickMode={isRigging} brushRadius={data.brushState.radius} onCamera={ws.setCamera} rigBendPoints={data.rigState.bendPoints} onSculpt={isRigging ? ws.placeRigBendPoint : ws.applyBrushAt} onFrame={ws.resetView} />}</div>
      <div className="edge">{surfaceMode === 'render' ? <button onClick={() => setSurfaceMode('edit')}>Back to Edit</button> : <><button onClick={ws.resetView}>Frame Object</button><button onClick={ws.resetView}>Reset View</button></>}</div>
      <GameReadyScanPanel scan={gameReadyScan} build={data.gameReadyBuild} />
      <div className="quality">{surfaceMode === 'render' ? 'RenderEngin Stage Active' : `Mesh Quality: ${data.mesh?.quality ?? 'Waiting'}`}</div>
      {surfaceMode === 'edit' && isRigging && <div className="sculpt"><div className="tools">{(['humanoid','quadruped','vehicle','creature','bird','fish','custom'] as const).map((target) => <button key={target} data-active={data.rigState.target === target} onClick={() => ws.setRigTarget(target)}>{target}</button>)}<button onClick={ws.undoRigBendPoint}>Undo Bend</button><button onClick={ws.clearRigMetadata}>Clear Rig Metadata</button></div><div className="sliders"><label>Bend Points<strong>{data.rigState.bendPoints.length}</strong></label><label>Next Joint<strong>{data.rigState.skeleton.bones[Math.min(data.rigState.bendPoints.length, data.rigState.skeleton.bones.length - 1)]?.name ?? 'done'}</strong></label></div></div>}
      {surfaceMode === 'edit' && data.processingStatus === 'editing' && <div className="sculpt"><div className="tools">{(['push','pull','smooth','inflate','carve','flatten'] as const).map((tool) => <button key={tool} data-active={data.activeTool === tool} onClick={() => ws.setTool(tool)}>{tool}</button>)}<button onClick={ws.undo}>Undo</button><button onClick={ws.redo}>Redo</button></div><div className="sliders"><label>Brush Radius<input type="range" min="0.05" max="0.6" step="0.01" value={data.brushState.radius} onChange={(e) => ws.setBrush({ radius: Number(e.currentTarget.value) })} /></label><label>Strength<input type="range" min="0.01" max="0.2" step="0.01" value={data.brushState.strength} onChange={(e) => ws.setBrush({ strength: Number(e.currentTarget.value) })} /></label></div></div>}
      {surfaceMode === 'edit' && <div className="download"><button disabled={!canDownloadGlb} onClick={() => ws.download('glb')}>.glb</button><button disabled={!canDownloadGlb || data.rigState.bendPoints.length === 0} onClick={() => ws.download('rig-metadata-glb')}>GLB + Rig Metadata</button><button disabled={!canDownloadObj} onClick={() => ws.download('obj')}>.obj</button></div>}
      <p className="hint">{surfaceMode === 'render' ? 'RenderEngin is mounted directly in ContentEngin · pinch and drag on the stage to inspect the asset' : 'MVP supports indexed binary GLB mesh primitives · Edit: drag to sculpt · Rig metadata: tap bend points · two fingers pan/zoom'}</p>
    </section>
  </main>;
}
