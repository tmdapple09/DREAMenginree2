...
import { CONTENTENGIN_2026_UPGRADES } from '@/engins/contentengin/upgradeMatrix';
...
export default function ImplicitAssetWorkspace({ onBack }: { onBack?: () => void }) {
  ...
  const upgradeCount = CONTENTENGIN_2026_UPGRADES.length;
  const liveUpgradePreview = CONTENTENGIN_2026_UPGRADES.slice(0, 4);
  ...
  `}</style>
    ...
    <section className="stage">
      ...
      <aside className="upgrade-deck" aria-label="ContentEngin 2026 upgrade status"><h2>2026 mobile-first upgrades · {upgradeCount}</h2><ul>{liveUpgradePreview.map((upgrade) => <li key={upgrade.id}><strong>{upgrade.title}</strong>{upgrade.userBenefit}</li>)}</ul></aside>
