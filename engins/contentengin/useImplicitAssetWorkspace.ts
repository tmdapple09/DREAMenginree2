...
export function useImplicitAssetWorkspace(ownerId = 'local-user', runtimeId = 'contentengin-runtime') {
  ...
  const process = useCallback(() => { const sourceImage = workspaceRef.current.data.sourceImage; if (!sourceImage) return; const token = processTokenRef.current + 1; processTokenRef.current = token; emit('contentengin:asset-process-requested', { sourceName: sourceImage.name }); updateData({ processingStatus: 'processing', visibleMessage: 'Shaping your image into a 3D asset…' }); window.setTimeout(() => { if (processTokenRef.current !== token || workspaceRef.current.data.sourceImage !== sourceImage) return; try { const editable = processImageToEditableMesh(sourceImage); if (processTokenRef.current !== token || workspaceRef.current.data.sourceImage !== sourceImage) return; const ready = editable.quality === 'Clean' || editable.quality === 'Open Surface' || editable.quality === 'Auto-fix applied'; updateData({ mesh: editable, previewMesh: meshToSnapshot(editable.mesh, editable.diagnostics), editHistory: [], redoStack: [], processingStatus: ready ? 'ready-to-download' : 'generated', visibleMessage: messageForQuality(editable.quality) }); emit('contentengin:asset-process-completed', { processingStatus: ready ? 'ready-to-download' : 'generated', meshDiagnostics: diagnosticsMetadata(editable), quality: editable.quality }); } catch { if (processTokenRef.current === token) updateData({ processingStatus: 'failed', visibleMessage: 'Processing failed. Try a simpler image.' }); } }, 16); }, [emit, updateData]);
