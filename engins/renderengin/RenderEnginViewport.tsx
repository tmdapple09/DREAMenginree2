...
import { createParsedObjRenderAsset, estimateRenderAssetMemory } from './assets';
import {
  requestWebGpuDevice,
  WebGpuRenderEngin,
...
export default function RenderEnginViewport({ runtime }: { runtime?: EnginRuntime<RenderIntent> }): React.JSX.Element {
  ...
  async function importObjFile(file: File): Promise<void> {
  ...
  return (
    <section style={{ display: 'grid', gap: 12 }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        ...
        <button type="button" onClick={() => { setSelectedObjectId('object:demo-triangle'); runtime?.dispatch({ type: 'render.object.select', payload: { id: 'object:demo-triangle' } }); }} style={{ border: '1px solid #bae6fd', borderRadius: 999, padding: '8px 12px', fontWeight: 900 }}>Select object</button>
        ...
        <button type="button" onClick={() => fileInputRef.current?.click()} style={{ border: '1px solid #38bdf8', borderRadius: 999, padding: '8px 12px', fontWeight: 900 }}>Import OBJ</button>
        <input ref={fileInputRef} type="file" accept=".obj,model/obj,text/plain" hidden onChange={(event) => { const file = event.target.files?.[0]; if (file) void importObjFile(file); }} />
      </div>
      <canvas
        ref={canvasRef}
        width={960}
        ...
        onClick={() => { setSelectedObjectId('object:demo-triangle'); runtime?.dispatch({ type: 'render.object.select', payload: { id: 'object:demo-triangle' } }); }}
