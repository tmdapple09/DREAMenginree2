...
export function validateMesh(mesh: Mesh, areaEpsilon = 1e-18): MeshDiagnostics { const edgeUse = new Map<string, number>(); let degenerateTriangles = 0; for (let i = 0; i + 2 < mesh.indices.length; i += 3) { const [a,b,c] = [mesh.indices[i], mesh.indices[i+1], mesh.indices[i+2]]; if (a >= mesh.vertices.length || b >= mesh.vertices.length || c >= mesh.vertices.length) throw new Error('Mesh contains an out-of-range triangle index.'); if (a === b || b === c || c === a || areaSq(mesh.vertices[a], mesh.vertices[b], mesh.vertices[c]) <= areaEpsilon) { degenerateTriangles++; continue; } for (const [u,v] of [[a,b],[b,c],[c,a]]) { const e = u < v ? `${u}:${v}` : `${v}:${u}`; edgeUse.set(e, (edgeUse.get(e) ?? 0) + 1); } } let boundaryEdges = 0; let nonManifoldEdges = 0; for (const use of edgeUse.values()) { if (use === 1) boundaryEdges++; else if (use > 2) nonManifoldEdges++; } return { vertices: mesh.vertices.length, triangles: Math.floor(mesh.indices.length / 3), degenerateTriangles, boundaryEdges, nonManifoldEdges }; }
