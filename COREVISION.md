...
- **Engins and DreamWindows**: Engins are the building-block modules (e.g. GameEngin, CodeEngin, MediaEngin) defined in `engine/engin-*` or `components/panels`. Each Engin exports a manifest and code that the engine can load (possibly via dynamic import or Module Federation). When instantiated, an Engin appears in a **DreamWindow** (a movable UI panel). The code uses `useEnginBridge` and similar hooks to connect an Engin instance to the global bus.  
