...
export default function CartridgeBrowser({
  initialCategory = ALL,
    ...
    >
      ...
      >
        ...
        <header style={{ marginBottom: 22 }}>
          ...
          >
            Pick a game and launch it from the GameEngin shelf. Older URLs at{" "}
            <code>/engines/games/library</code> still work — this is the new
            canonical home.
          </p>
        </header>
        ...
        >
          {filtered.length} of {CARTRIDGE_MANIFEST.length} cartridge
          ...
          TIER_ORDER.map((t) => {
            ...
            return (
              <section key={t} style={{ marginBottom: 30 }}>
                ...
                >
                  {items.map((c) => (
                    <CartridgeCard key={c.id} cartridge={c} />
                  ))}
                </div>
              </section>
            );
...
function CartridgeCard({
  ...
  cartridge: CartridgeManifestEntry;
}) {
  return (
    <Link
