...
function getMockOutput(
  ...
  if (engine === "lab") {
    const simResult = hasParticle
      ? "1024 particles, avg v=12.4m/s, energy=0.83J"
      : hasFluid
        ? "Flow stable Re=4200, viscosity=0.001Pa·s"
        : hasNeural
          ? "Convergence: 0.003, epochs=100, accuracy=97.2%"
          : hasQuantum
            ? "Fidelity: 0.94, depth=12, gates=24"
            : "42 data points processed, σ=2.31";
    return [
      `[${ts()}] LabEngin workspace opened`,
      `[${ts()}] Experiment: code_dream_${Date.now()}`,
      `[${ts()}] Simulation workspace prepared`,
      `[${ts()}] Running analysis…`,
      `[${ts()}] Result: ${simResult}`,
      `[${ts()}] Result package prepared`,
  ...
  if (engine === "asset") {
    return [
      `[${ts()}] AssetEngin workspace opened`,
  ...
  switch (language) {
    case "python":
      return [
        `Python 3.12.0 [DREAMengin runtime]`,
        `>>> Executing…`,
        `BPM: 128`,
        `Game score: 9999`,
        `>>> Done.`,
    ...
    case "javascript":
      return [
        `Node.js v22 [DREAMengin runtime]`,
        `> Executing…`,
        `[{"game":"platformer","score":9999}]`,
        `> Done.`,
      ];
    case "typescript":
      return [
        `TypeScript 5.7.2 [DREAMengin runtime]`,
        `> Compiling…`,
        `✓ Compiled in 120ms`,
        `{ result: "success" }`,
        `> Done.`,
    ...
    case "bash":
      return [
        `bash 5.2 [DREAMengin runtime]`,
        `$ Executing…`,
        `✓ 291 tests passed in 1.24s`,
        `$ Exit 0`,
      ];
  }
}
...
function AsciiBarChart({
  ...
  return (
    <div
      style={{ display: "flex", gap: 8, alignItems: "flex-end", height: 60 }}
    >
...
export default function CodeDreamIDE() {
  ...
  const handleStop = useCallback(() => {
    ...
    setOutputLines((prev) => {
      const next = [
        ...prev,
        `[${new Date().toISOString().slice(11, 19)}] ⛔ Interrupted by user`,
      ];
      dualRuntimeBridge.emit("code", "code:output", {
        lines: next,
        status: "error",
  ...
  const handleEamsAssist = useCallback(async () => {
    ...
    try {
      ...
      setEamsReply(
        "⚠️ Dr. Eams is unavailable right now. Check your connection.",
      );
    } finally {
  ...
  const activeEngine =
    ENGINE_CONNECTIONS.find((e) => e.id === engine) ?? ENGINE_CONNECTIONS[0];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
        ...
        <div className="de-widget-header">
          ...
          <span
            style={{
              ...
              fontWeight: 700,
              background: `${activeEngine.accent}15`,
              padding: "2px 8px",
              borderRadius: 5,
            }}
          >
            ...
            {ENGINE_CONNECTIONS.map((eng) => (
              <button
                ...
                style={{
                  ...
                  background:
                    engine === eng.id ? eng.color : "rgba(255,255,255,0.55)",
              ...
              >
                <span
                  style={{
                    color:
                      engine === eng.id ? eng.accent : "var(--de-text-dim)",
                  }}
                >
                  {eng.icon}
                </span>
      ...
      <div className="de-widget" style={{ marginBottom: 12 }}>
        ...
        <div className="de-widget-header" style={{ gap: 8, flexWrap: "wrap" }}>
          ...
          <button
            ...
            title={
              swapped
                ? "Preview left · Editor right — click to swap back"
                : "Editor left · Preview right — click to swap"
            }
            style={{
              display: "flex",
              alignItems: "center",
        ...
        >
          ...
          {swapped && (
            ...
            >
              ...
              >
                ...
                {previewMode === "game" && (
                  <div style={{ padding: 4 }}>
                    ...
                    >
                      Scene:{" "}
                      {outputLines.length > 0 ? "scene_active" : "waiting…"}
                      <br />
                      Entities: {outputLines.length > 0 ? "124" : "0"}
                      <br />
                      Draw calls: {outputLines.length > 0 ? "24" : "—"}
                      <br />
                      Physics:{" "}
                      {engine === "game" && outputLines.length > 0
                        ? "Havok ●"
                        : "idle"}
                      <br />
                      <br />
                ...
                {previewMode === "canvas" && (
                  <div style={{ padding: 4 }}>
                    ...
                    >
                      Render output · {language} ·{" "}
                      {engine !== "none" ? activeEngine.label : "standalone"}
                    </div>
                  </div>
                )}
              </div>
              ...
              >
                <span style={{ color: activeEngine.accent }}>
                  {activeEngine.icon}
                </span>
                ...
                {liveMode ? (
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background:
                        status === "running" ? "rgba(99,102,241,0.15)" : ACCENT,
      ...
      <div className="de-widget" style={{ marginBottom: 12 }}>
        ...
        <div className="de-widget-body">
          ...
          >
            Ask Dr. Eams to explain, refactor, or generate code. Vocabulary is
            always recognised — try <em>"explain closure"</em>,{" "}
            <em>"write a class called Player"</em>, or <em>"add try-except"</em>
            .
          </p>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") void handleEamsAssist();
              }}
            ...
            <button
              ...
              style={{
                ...
                fontSize: 12,
                fontWeight: 700,
                background: eamsLoading
                  ? "rgba(167,139,250,0.15)"
                  : "rgba(167,139,250,0.85)",
                color: "#fff",
                border: "none",
                cursor:
                  eamsLoading || !eamsPrompt.trim() ? "not-allowed" : "pointer",
                opacity: !eamsPrompt.trim() ? 0.4 : 1,
                display: "flex",
                alignItems: "center",
                gap: 5,
                transition: "all 0.15s",
