...
Because `FILE_TREE.md` uses ASCII tree prefixes, parse each line structurally. Work with prefixes like `âââ`, `âââ`, and `â` to detect folder/file levels. Count the leading characters before the node marker to identify depth. Nesting will usually appear through combinations of spaces and `â`, so locate the position of `â` or `â` in each line and use that index to determine indentation level. Simulate a stack to track parent folders and reconstruct full paths from root to each node.
...
â What it becomes
â Where it lives
â What tech improves it
â How to use that tech
â Goal
```
...
One concept
âââ one model
    âââ one source of truth
        âââ many render surfaces
```
...
Profile
âââ renders as:
    âââ DreamR Profile
    âââ Edit Profile
    âââ HomeDream Profile Card
    âââ Settings Profile
    âââ Public Profile
    âââ Shared Dream Identity Surface

Feed
âââ renders as:
    âââ DreamR Feed
    âââ HomeDream Feed Widget
    âââ Profile Feed
    âââ Shared Dream Activity Feed
    âââ Notification / Activity Surface
    âââ DreamSpace Social Surface

Post
âââ renders as:
    âââ Feed Card
    âââ Profile Post
    âââ HomeDream Object
    âââ DreamR Object
    âââ Shared Dream Object
    âââ Movable Dream / Media Object
...
âââ renders as:
    ...
    âââ Tool
    âââ Media Object
    âââ Profile Surface
    âââ Feed Object
    âââ Shared Object
    âââ Spatial Object
    âââ Environment
    âââ Ruleset
```
...
Important: do not split Dreams into separate architectures for âsystem dreamsâ and âuser dreams.â
...
```txt
Capability
âââ powers a user-facing behavior
    âââ produces or controls Dreams / modules / surfaces
        âââ appears through HomeDream, DreamSpace, DreamR, DreamDMBar, GameEngin, Shared Dreams, or another shared runtime surface
```
...
```txt
Feed
âââ shared Feed/Post/Activity source of truth
    âââ DreamR Feed
    âââ HomeDream Feed Widget
    âââ Profile Feed
    âââ DreamSpace Social Surface
    âââ Shared Dream Activity Feed
    âââ Notification / Activity Surface
    âââ DreamDMBar context/actions
```
...
```txt
GameEngin
âââ loads Game Cartridges
    âââ produces playable Games
        âââ controlled by GameRemote
        âââ rendered by Babylon/WebGPU when needed
        âââ hosted as Dreams/modules/windows/surfaces
        âââ persisted/shared through the normal DREAMengin runtime
```
...
â Becomes: the front door into DREAMengin + DreamR. Not just a marketing page â the first surface of the creative OS.
...
Goal: make users understand: âThis is a creative OS/world, and DreamR is the human social layer inside it.â

---

## 2. DreamR

â Becomes: the social layer of DREAMengin. Human social media where creativity and individuality drive visibility.
...
â Becomes: the shared creative discovery and activity system.
...
â Becomes: the shared identity model and user-facing identity surface.
...
â Becomes: the command/social/input nervous system.
...
â Becomes: native communication inside the creative OS.
...
â Becomes: conversation attached to creative objects.
...
â Becomes: the userâs personal creative desktop/home world.
...
Goal: HomeDream becomes the userâs personal operating surface where tools, games, media, DreamR, DreamDMBar, widgets, and Dreams live.

---

## 9. DreamSpace

â Becomes: the expanded spatial/world surface.
...
â Becomes: the core user-facing object model of DREAMengin.
...
```txt
Dream
âââ can render as DreamWindow
```
...
â Becomes: focused creation modes.
...
â Becomes: a shared living creative environment.
...
â Becomes: one render/container mode for Dreams.
...
â Becomes: the active building blocks of the OS.
...
â Becomes: one connected runtime across HomeDream and DreamSpace.
...
â Becomes: native game/playable runtime layer inside DREAMengin.
...
â Becomes: the user-facing playable experiences inside DREAMengin.
...
â Becomes: the package format for games and playable runtime experiences.
...
â Becomes: the proof cartridge / reference game for the GameEngin system.
...
â Becomes: universal game/input controller.
...
â Becomes: the visual/world/game engine layer.
...
â Becomes: premium graphics/performance layer.
...
â Becomes: small movable Dreams/tools/objects.
...
â Becomes: portable creative objects.
...
â Becomes: creation/publishing engine.
...
â Becomes: coding/workbench surface.
...
â Becomes: audio/music creation surface.
...
â Becomes: the user-facing identity, theme, and visual-brand capability.
...
â Becomes: the experimental user-facing lab for DREAMengin.
...
â Becomes: the simulation/rules/world-behavior engine.
...
```txt
SimEngin
âââ powers behavior/rules/simulation for:
    âââ Dreams
    âââ GameEngin
    âââ DreamSpace
    âââ Shared Dreams
...
â Becomes: DREAMenginâs OS skin.
...
â Becomes: direct physical-feeling control layer.
...
â Becomes: creator object economy.
...
â Becomes: bridges to outside services.
...
â Becomes: world event awareness.
...
â Becomes: a user-facing creative operating world.
...
Goal: DREAMengin becomes apps + games + chats + tools + media + Dreams + creative surfaces as movable building blocks inside the userâs own digital world.
